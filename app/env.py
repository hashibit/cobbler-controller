...
envs_git_repo = "http://%s:%s@gitlab.sz.mycorp.com/rainy/infra/env.git" % (
    git_username,
    git_password,
)
...
def get_env(env_name):
    ...
    path_license_config = os.path.join(
        envs_dir, env_name, "license-ca", "config", "config.yml"
    )
...
def deploy_env_log(env_name):
    ...
    logger_file = "/tmp/hub-deploy.%s.log" % env_name
    p = subprocess.Popen(
        ["tail", logger_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
